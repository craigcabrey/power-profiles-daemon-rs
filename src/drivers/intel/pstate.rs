...
impl Driver {
    ...
    fn turbo_enabled(&self) -> Result<bool> {
        match fs::read_to_string(Self::NO_TURBO_FLAG).with_context(|| format!("Failed to read from {}", Self::NO_TURBO_FLAG)) {
...
impl FromStr for Status {
    ...
    fn from_str(s: &str) -> Result<Self> {
        match s {
            ...
            _ => Err(anyhow::anyhow!("Unrecognized intel-pstate driver status {}", s)),
        }
    }
}